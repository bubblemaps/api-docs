...
- The token chain and address, through path parameters. If you provide a non-public chain, you'll get a `400` error (see [Get Supported Chains](/docs/api/endpoints/get-supported-chains)).
...
Query duration ranges from a few `ms` for cache hits, up to a minute for outliers. Most queries should not take more than `10s` though.
